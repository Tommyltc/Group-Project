...
var ytEmbed = {
    ...
    getCard: function(object){
        var id = object.id;
        // '<div class="card"><img class="card-img-top" src="'+object.thumbnail.url+'" alt="Card image cap"<div class="card-body"><h4 class="card-title"><a>'+object.title+'</a></h4><p class="card-text">'+object.description+'</p></div>';
        var card = '<div class="card" style="width: 18rem;cursor: pointer;" onclick="ytEmbed.cardClick(\''+id+'\')"><img class="card-img-top" src="'+object.thumbnail.url+'"><div class="card-body"><div class="card-title h5">'+object.title+'</div><div class="card-subtitle h6">'+object.description+'</div></div><div class="small p-1 pl-3 bg-youtube text-white card-footer"><b>Youtube</b> on '+new Date(object.publishedAt).toString().split("GMT",1)+'</div></div>'
        return card;
    },
    ...
    listVideos: function(json, cfg) {
        ...
        for (var i = 0; i < json.items.length; i++) {
            var item = {};
            item.title = json.items[i].snippet.title
            item.id = json.items[i].id;
            item.description = json.items[i].snippet.description;
            item.publishedAt = json.items[i].snippet.publishedAt;
            item.thumbnail = json.items[i].snippet.thumbnails.standard;
            itemList.push(item);
        }
        console.log(itemList);

        this.jsonCallback(itemList);
        // this.cfg = cfg;
        // if (!this.cfg.player) {
